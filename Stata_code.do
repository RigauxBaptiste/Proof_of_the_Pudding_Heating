...
* Upper panel: Household characteristics 
    ...
    gen total_number_hh = 0 
    replace total_number_hh = Q641_1 + Q641_2 + Q641_3
	
    capture drop number_kids_l6yo
    gen number_kids_l6yo = 0 
    replace number_kids_l6yo = Q641_2
    ...
    *
    ...
    capture drop respondent_flag 
    capture drop respondent_flag_kids 
    ...
    *
    ...
    gen respondent_flag = !missing(Q641_1) | !missing(Q641_2) | !missing(Q641_3) | !missing(Q641_4)
    sum respondent_flag
    scalar num_resp = round(r(sum), 0.01)

    sum number_kids_l6yo if !missing(Q641_2)
    scalar avg_kids_l6yo = round(r(mean), 0.01)

    gen respondent_flag_kids = !missing(Q641_2)
...
* Middle panel: Participants' housing characteristics
    capture drop has_PV    
    gen has_PV = 0
    replace has_PV = 1 if strpos(Q06, "Zonnepanelen") > 0
	
	capture drop epc    
    gen epc = ""
    replace epc = "A" if Q6381_5 == "A" | Q6381_5 == "A+"
    replace epc = "A" if Q6381_4 < 100
...
program define App_T6_T7_manual_over
...
frame change default
capture frame drop man_overr_hs1

frame copy default man_overr_hs1 
capture frame change man_overr_hs1 
keep if (hs1_window == 1) & (reason_stop != 0 & reason_stop != -1 & reason_stop != -2)

*

gen rounded_t_in = round(t_in, 0.1)
gen rounded_total_duration_int_hour = round(total_duration_int_hour, 0.1)
gen temp_var = substr(tod, 1, 5)
gen rounded_tod = subinstr(temp_var, ":", "h", 1)

*

keep hh_id rounded_tod rounded_total_duration_int_hour rounded_t_in feedback_for_stop 
order hh_id rounded_tod rounded_total_duration_int_hour rounded_t_in feedback_for_stop 

* Placeholders for Heating and Water (not the final values imputed by hand)
gen Heating = 0
gen Water = 0

* Creating the dataset of manual overrules of HS1: 

format rounded_total_duration_int_hour %9.1f
format rounded_t_in %9.1f
tostring rounded_total_duration_int_hour, replace format(%9.1f)
tostring rounded_t_in, replace format(%9.1f)


export delimited hh_id rounded_tod rounded_total_duration_int_hour rounded_t_in feedback_for_stop Heating Water using "Table6.csv", replace

// Table 7: HS2

frame change default
capture frame drop man_overr_hs2

*

frame copy default man_overr_hs2 
frame change man_overr_hs2 
keep if (hs2_window == 1) & (reason_stop != 0 & reason_stop != -1 & reason_stop != -2)
gen rounded_t_in = round(t_in, 0.1)
gen rounded_total_duration_int_hour = round(total_duration_int_hour, 0.1)
gen temp_var = substr(tod, 1, 5)
gen rounded_tod = subinstr(temp_var, ":", "h", 1)
...
* Creating the dataset of manual overrules of HS2: 
...
export delimited hh_id rounded_tod rounded_total_duration_int_hour rounded_t_in feedback_for_stop Heating Water using "Table7.csv", replace
...
program define App_T8_reg_rebound_kWh
